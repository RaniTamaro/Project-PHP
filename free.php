    ...
    function show_free_rooms(){
		...
		while($row = mysqli_fetch_array($allRooms)){
			...
			foreach( $reservedRooms as $resId ){
				if ($row[0] == $resId) $isfree = false;
			}
			
			if ($isfree) $freeRooms += [$row];
		}
        ...
        <form method='POST'>
			...
			<input type="date" required class="form-control" id="checkinStartDateInput" name="freeRoomStartDate" value="<?=$_SESSION["freeRoomStartDate"]?>">
			</td><td>
		    <label for="checkinEndDateInput">Zalemdowanie do</label>
			<input type="date" required class="form-control" id="checkinEndDateInput" name="freeRoomEndDate" value="<?=$_SESSION["freeRoomEndDate"]?>">
			</td><td>
			<p> </p>
